...
function security(){
    ...
    echo -e "${BLUE}[+] ${WHITE}Aplicando configurações de segurança do Apache!${RESET}"

    #=======================================
    # Remover Directory Listing
    #=======================================

    if [[ -e "${apache_config}" ]]; then

        sed -i \
            's/Options[[:space:]].*/Options FollowSymLinks/' \
            "${apache_config}"

    fi

    #=======================================
    # Ocultar informações da versão do Apache
    #=======================================

    if [[ -e "${security_config}" ]]; then

        sed -i \
            -e 's/^[#[:space:]]*ServerTokens.*/ServerTokens Prod/' \
            -e 's/^[#[:space:]]*ServerSignature.*/ServerSignature Off/' \
            "${security_config}"
    ...
    if [[ -e "${apache_config}" ]]; then
        ...
        if ! grep -q 'AccessFileName .htaccess' "${apache_config}"; then

            cat <<CONFIG >> "${apache_config}"
...
CONFIG
    ...
    echo -e "${BLUE}[+] ${WHITE}Testando configuração do Apache!${RESET}"
    ...
    if apache2ctl configtest; then
        ...
        echo -e "${RED}[!] Erro na configuração do Apache!${RESET}"
        return 1
...
while [ $# -gt 0 ]; do
	case "$1" in
		-d) debug ; debug=1 ;;
        -h|--help) echo -e "Ajuda: $0 -h\n\t-d, --debug | Ativa o modo de depuração\n\t-s, --security | Ativa o modo seguro" ; exit 0 ;;
        -s|--security) security=1 ;;
...
#=======================================
# Ativando a configuração segura 
#=======================================
